...
.OUTPUTS
    System.String.
.NOTES
    Created by Octavian Cordos
.LINK
    https://SCCM.Zone/Export-CMDBToTelegraf
.LINK
...
.EXAMPLE
    Get-CMDBClientInfo -Server 'SomeServer' -Database 'CM_SomeSiteCode'
.NOTES
    This is an internal script function and should typically not be called directly.
.LINK
    https://SCCM.Zone
...
#>
    ...
    Begin {
        ...
        [string]$Query =
        "
            DECLARE @Date DATETIME = DATEADD(hh, -12, GETDATE());
            SELECT
                CMG_Update_Scan = (
